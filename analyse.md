...
- Selected-prefix consumption now prepares a local replay state, resets fcitx, and replays the remaining raw T9 digits when the committed candidate's normalized comment exactly equals the selected prefix, so examples beyond `yo ci` (such as `2496 -> ci -> one Hanzi`, then remaining `96`) can rebuild candidate rows from the remaining composition without splitting phrase candidates like `ci wo` or blanking the whole T9 overlay.
